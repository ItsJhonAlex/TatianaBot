...
class PokemonCommands(commands.Cog):
    ...
    @commands.command(name="pokedex", description="Muestra tu Pokédex")
    async def pokedex(self, ctx):
        user = get_user(ctx.author.id)
        pokemons = session.query(Pokemon).filter_by(user_id=user.id).all()
        
        if not pokemons:
            await ctx.send("No tienes ningún Pokémon en tu Pokédex.")
        else:
    ...
    async def mostrar_pokemon(self, ctx):
        if not self.update_attempts(ctx.author.id):
            attempt = session.query(Attempt).filter_by(user_id=get_user(ctx.author.id).id, attempt_type='pokemon').first()
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(f"{self.pokemon_api_url}{pokemon_id}") as response:
                    if response.status == 200:
                        ...
                        view = PokemonCatchView(self, pokemon_name, pokemon_id, catch_rate)
